...
```bash
XTTS_CONFIG_FILE=/путь/до/вашего/config.yaml PYTHONPATH=src python -m xtts_stream.api.service.balancer
```
...
```
Клиент ─WS─> [Balancer] ──> [Worker/WS → XTTS.inference_stream] → PCM
                                    │
                                    └─ Pacer + Time‑Shard → Base64(JSON) → Клиент
```
...
Переменные окружения: только `XTTS_CONFIG_FILE` (путь к `config.yaml`).
...
| ---------------------- | ----: | -----------: | ---------------------------------------------------------------------------- |
| `output_format`        |   str |  `pcm_24000` | **Жёсткий** параметр формата. Поддержка: только `pcm_24000` (совпадает с моделью) |
